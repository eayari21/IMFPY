       ...
       real l_p 
       real*8 q 
       common /params/ B_0,pm,q,r_e,dpm,c,pi,U,R,r0,phi0,theta,B_R,B_T
       q=1
       l_p=pi/2.
       pm=1
       pm=1
...
! Itikawa Mason et al, table 11
      ...
      call fields(x,y,z,vx,vy,vz,bx,by,bz,t2) 
! Lorentz force 
       fxl=q*(vy*bz-vz*by)/c 
       fyl=q*(vz*bx-vx*bz)/c 
       fzl=q*(vx*by-vy*bx)/c 
      ...
      subroutine fields(x,y,z,vx,vy,vz,bx,by,bz,t2) 
      implicit real*8 (a-h,o-z) 

      common /params/ B_0,pm,q,r_e,dpm,c,pi,U,R,r0,phi0,theta,B_R,B_T 

! define our B-field here
! # || IMF
! # %% Function to calculate the magnetic field components
! def magnetic_field(x, y, U, R, B0, r0, phi0, theta, t):
!    rsq = x**2 + y**2
!    B_R = B0 * (r0 / rsq)**2 * np.cos(pi * t / 11 + phi0)
!   B_T = B0 * (r0 / rsq) * np.cos(theta) * np.cos(pi * t / 11 + phi0)
...
! aligned, centered dipole
       ...
       r=sqrt(r2) 
       r3=r**3
       B_R=B0*(r0/r)**2*cos(pi*t2/11+phi0)
       B_T=B0*(r0/r)*cos(theta)*cos(pi*t2/11+phi0)

       bx=U*y+B_R*x/r-B_T*y/r
       by=-U*x+B_R*y/r+B_T*x/r
       bz=0
